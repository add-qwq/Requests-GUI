...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        headers_layout.addWidget(QLabel("Headers (键值对，每行一个)"))
        self.headers_table = QTableWidget(3, 2)  # 3行2列（键、值）
        ...
        self.params_tab.setPlaceholderText("输入查询参数（键=值&键=值，或 JSON）")
        self.data_tab = QTextEdit()
        self.data_tab.setPlaceholderText("输入请求体（文本或 JSON）")
        self.tab_widget.addTab(self.params_tab, "查询参数")
        self.tab_widget.addTab(self.data_tab, "请求体")

        response_layout = QVBoxLayout()
        self.status_label = QLabel("状态码：")
    ...
    def handle_response(self, result):
        if result.get("error"):
            self.status_label.setText(f"状态码：错误 - {result['error']}")
            self.response_headers.clear()
            self.response_content.setText(f"错误信息：{result['error']}")
            return
